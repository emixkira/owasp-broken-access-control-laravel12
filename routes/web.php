...
Route::middleware(['auth'])->group(function () {
    ...
    */
    ...
    Route::post(
        '/users/name/change',
        [UserController::class, 'changeName']
    )->name('change.name');
    ...
    Route::post(
        '/users/img/change',
        [UserController::class, 'changeImg']
    )->name('change.img');

    Route::get(
        '/download-privacy',
        [UserController::class, 'download']
    )->name('download');

    /*
    |--------------------------------------------------------------------------
    | FILE UPLOAD - SECURITY MISCONFIGURATION
    |--------------------------------------------------------------------------
    */
    ...
    */
    ...
    Route::middleware(['admin'])
        ...
        ->group(function () {
            ...
            Route::post(
                '/users/{id}/toggle',
                [AdminController::class, 'toggleUsersAdmin']
            )->name('admin.users.toggle');

            Route::post(
                '/articles/{id}/toggle',
                [AdminController::class, 'toggleArticleStatus']
            )->name('admin.articles.toggle');

            // UNSECURE - VULNERABILE A SSRF
            Route::get(
                '/fetch-external-data',
                [AdminController::class, 'fetchExternalData']
            )->name('admin.fetch-external-data');
        });
